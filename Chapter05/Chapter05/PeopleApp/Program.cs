...
WriteLine(
	format: "{0}'s favorite wonder is {1}. Replace Its integer is {2}.",
	arg0: bob.Name,
...
WriteLine($"{bob.Name}'s buket list is {bob.BucketList}");
...
Book book = new(isbn: "978-1803237800",
	title: "C# and .NET 7 - Modern Cross-Platform Development Fundamentals")
{
	Author = "Mark J. Price",
	PageCount = 821
};

WriteLine("{0}: {1} Writtin by {2} has {3:N0} pages.",
...
if (zillah + lamech)
{
	WriteLine($"{zillah.Name} and {lamech.Name} succesfully got married.");
}
...
try
{
	...
	{
	WriteLine($"{ex.GetType()} says: {ex.Message} number {number}" );
}
...
ImmutableVehichle repaintedCar = car with
{
